...
router.post('/fileUpload', verifyToken, verifyDuplicateLogin, verifyIsAdmin, (req, res, next)=>{
    try{
        Organization.destroy({where:{}}); // 엑셀파일이 업로드 될 때마다 기존 데이터는 모두 삭제한다.
        const resData = {};
...
router.get('/category/:categoryNum', verifyToken, verifyDuplicateLogin, verifyIsAdmin, async (req, res, next)=>{
    try{
        let category = {
            0: "한국전력공사",
            1: "한국전기안전공사",
